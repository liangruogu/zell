...
const tools: AgentConfig['tools'] = [
  ...
  {
    name: 'list_articles',
    description: '列出知识库中所有文章的标题和内容预览',
    parameters: { type: 'object', properties: {}, required: [] },
    execute: async () => {
      const project = useProjectStore.getState().currentProject
      if (!project) return '当前没有打开的项目。'
  ...
  },
  {
    ...
    description: '全文搜索知识库文章内容',
    parameters: { type: 'object', properties: { query: { type: 'string', description: '搜索关键词' } }, required: ['query'] },
    execute: async (args) => {
      const project = useProjectStore.getState().currentProject
      if (!project) return '当前没有打开的项目。'
      const results = await invoke<SearchResult[]>('search_knowledge', { projectId: project.id, query: args.query as string, limit: 5 })
  ...
  },
  {
    ...
    description: '搜索外部资源（PDF、Word文档、PPT、网页提取文本）',
    parameters: { type: 'object', properties: { query: { type: 'string', description: '搜索关键词' } }, required: ['query'] },
    execute: async (args) => {
      const project = useProjectStore.getState().currentProject
      if (!project) return '当前没有打开的项目。'
      const results = await invoke<SearchResult[]>('search_resources', { projectId: project.id, query: args.query as string, limit: 5 })
  ...
  {
    name: 'get_article',
    description: '获取指定知识库文章的完整内容',
    parameters: { type: 'object', properties: { id: { type: 'string', description: '文章ID' } }, required: ['id'] },
    execute: async (args) => {
      const article = await invoke<KnowledgeArticle>('get_knowledge_article', { id: args.id as string })
      return JSON.stringify({ title: article.title, content: article.content })
    },
  },
  {
    name: 'get_resource',
    description: '获取外部资源的完整提取文本',
    parameters: { type: 'object', properties: { type: { type: 'string', enum: ['file', 'link'], description: '资源类型' }, id: { type: 'string', description: '资源ID' } }, required: ['type', 'id'] },
    execute: async (args) => {
      const resource = await invoke<ResourceContent>('get_resource_content', { resourceType: args.type as string, id: args.id as string })
...
export function createKnowledgeAgentConfig(modelId?: string): AgentConfig {
  return { systemPrompt: KNOWLEDGE_SYSTEM_PROMPT, tools, modelId: modelId || '' }
}
