...
async function syncArticles(serverUrl: string, projectId: string, token: string) {
    try {
        ...
        for (const a of articles) {
            ...
            } catch (e) { logger.error('Failed to create synced article', e) /* article might already exist */ }
        }
    } catch (e) { logger.error('Failed to sync articles from server', e) /* server offline, skip */ }
}
...
export default function HomePage() {
    ...
    const handleJoin = useCallback(async () => {
        ...
        try {
            ...
            if (!res.ok) {
                const err = await res.json().catch((e) => { logger.error('Failed to parse join response', e); return { error: '未知错误' } })
            ...
            let projectName = `${data.project_name || data.project_id.slice(0, 8)} (协作)`
            let suffix = 1
            while (projects.some((p) => p.name === projectName)) {
                suffix++
                projectName = `${data.project_name || data.project_id.slice(0, 8)} (协作 ${suffix})`
            }
            await createProject({
                id: data.project_id,
                name: projectName,
            ...
            // [Error] [2026-07-31T12:11:20.418Z] ERROR Failed to join project UNIQUE constraint failed: projects.id
            // (anonymous function) (user-script:8:83)
            // write (logger.ts:37)
            // (anonymous function) (HomePage.tsx:128)
            logger.error('Failed to join project', e)
