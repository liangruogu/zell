    ...
    <div className="space-y-3">
      ...
      {(el.type === 'rect' || el.type === 'ellipse') && (
        <>
          ...
          {el.type === 'rect' && <div><label className="text-[10px] text-gray-500">圆角</label><input type="number" value={el.props.borderRadius || 0} min={0} max={100} onChange={e => updateProps({ borderRadius: +e.target.value })} className="w-full px-1.5 py-0.5 text-xs border border-gray-200 rounded" /></div>}
        </>
      )}
    </div>
  )
